...
router.get('/paquete', async (req, res, next) => {
    
	paqueteSchema.find(function(err, paquete) {
		if (err){
      return err;
    } 
		res.send(paquete);
...
router.delete('/paquete/:_id', async (req, res, next) => {
    ...
    paqueteSchema.findByIdAndRemove(req.params._id, (err, paquete) => {
    if (err) {
      console.log('Error: ', err);
    }
    console.log('Paquete eliminado: ', paquete);
  });
});
